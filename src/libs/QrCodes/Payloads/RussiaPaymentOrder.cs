...
/// <summary>
/// Specification of RussianPaymentOrder <br/>
/// https://docs.cntd.ru/document/1200110981 <br/>
/// https://roskazna.gov.ru/upload/iblock/5fa/gost_r_56042_2014.pdf <br/>
/// https://sbqr.ru/standard/files/standart.pdf <br/>
/// <br/>
/// Specification of data types described in the above standard <br/>
/// https://gitea.sergeybochkov.com/bochkov/emuik/src/commit/d18f3b550f6415ea4a4a5e6097eaab4661355c72/template/ed <br/>
/// <br/>
/// Tool for QR validation <br/>
/// https://www.sbqr.ru/validator/index.html <br/>
/// </summary>
public class RussiaPaymentOrder
{
    ...
    public byte[] ToBytes()
    {
        ...
        if (bytesOut.Length > 300)
            throw new InvalidOperationException($"Data too long. Payload must not exceed 300 bytes, but actually is {bytesOut.Length} bytes long. Remove additional data fields or shorten strings/values.");
        return bytesOut;
    }
    ...
    private string DetermineSeparator()
    {
        ...
        throw new InvalidOperationException("No valid separator found.");
    }
    ...
    {
        return ValidateInput(input, fieldName, [pattern], errorText);
    }
        ...
        string? errorText = null)
    {
        if (input == null)
        {
            throw new ArgumentNullException(fieldName, $"The input for '{fieldName}' must not be null.");
        }
        ...
        foreach (var pattern in patterns)
        {
            ...
            {
                throw new ArgumentException(errorText ?? $"The input for '{fieldName}' ({input}) doesn't match the pattern {pattern}");
            }
        }
