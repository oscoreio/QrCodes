...
public class FastPngRenderer : IRenderer
{
        ...
        bool drawQuietZones = true)
    {
        ...
        var size = (data.ModuleMatrix.Count - (drawQuietZones ? 0 : 8)) * pixelsPerModule;
        png.WriteHeader(size, size, 1, PngBuilder.ColorType.Greyscale);
        png.WriteScanlines(DrawScanlines(data, pixelsPerModule, drawQuietZones));
        png.WriteEnd();
        return png.GetBytes();
    }
    ...
    /// </summary>
    private static byte[] DrawScanlines(
        QrCode data,
        int pixelsPerModule,
        bool drawQuietZones)
    {
